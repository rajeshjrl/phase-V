...
class Blog_Model extends CI_Model {
    ...
    public function getCategories($fields = '', $condition_to_pass = '', $order_by_to_pass = '', $limit_to_pass = '', $debug_to_pass = 0) {
        if ($fields == '') {
            ...
            $fields = "c.*,IF(c.parent_id > 0,(select category_name from " . $this->db->dbprefix('mst_blog_categories') . " c2 ";
            $fields.="where c2.category_id = c.parent_id limit 1),'-') as parent_category,(select `url` from ";
            $fields.=$this->db->dbprefix('mst_uri_map') . " u where u.type='blog-category' and u.rel_id=c.category_id) as page_url";
        }
